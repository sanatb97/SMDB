...
CREATE TABLE rating (movie_id varchar(5),no_of_reviews int,rev_stars float,public_rev_perc varchar(4),public_no_votes int,FOREIGN KEY(movie_id) REFERENCES movie(movie_id));
...
varchar(30),actor_gender varchar(6),act_dob varchar(10),PRIMARY KEY(actor_id));

CREATE TABLE producer(prod_id varchar(5) NOT NULL,prod_name varchar(40),PRIMARY KEY(prod_id));
...
CREATE TABLE acts_in(movie_id varchar(5),actor_id varchar(5),char_name varchar(30),FOREIGN KEY(movie_id) REFERENCES movie(movie_id));
...
CREATE TABLE produces(movie_id varchar(5),prod_id varchar(5),FOREIGN KEY(movie_id) REFERENCES movie(movie_id),FOREIGN KEY(prod_id) REFERENCES producer(prod_id));